...
impl Mapper for Mmc3 {
    ...
    fn save_state(&self, buff: &mut Vec<u8>) {
        ...
        save_u8(buff, self.bank_select);
        save_u8(buff, self.irq_counter);
        save_u8(buff, self.irq_reload);
        save_bool(buff, self.irq_reload_requested);
        save_bool(buff, self.irq_enabled);
        save_bool(buff, self.irq_flag);
        save_u8(buff, self.last_a12);
        save_u8(buff, self.filtered_a12);
        save_u8(buff, self.low_a12_counter);
    }

    fn load_state(&mut self, buff: &mut Vec<u8>) {
        load_u8(buff, &mut self.low_a12_counter);
        load_u8(buff, &mut self.filtered_a12);
        load_u8(buff, &mut self.last_a12);
        load_bool(buff, &mut self.irq_flag);
        load_bool(buff, &mut self.irq_enabled);
        load_bool(buff, &mut self.irq_reload_requested);
        load_u8(buff, &mut self.irq_reload);
        load_u8(buff, &mut self.irq_counter);
        load_u8(buff, &mut self.bank_select);
        load_bool(buff, &mut self.switch_prg_banks);
        load_bool(buff, &mut self.switch_chr_banks);
        load_usize(buff, &mut self.prg_bank_7);
        load_usize(buff, &mut self.prg_bank_6);
        load_usize(buff, &mut self.chr1_bank_5);
        load_usize(buff, &mut self.chr1_bank_4);
        load_usize(buff, &mut self.chr1_bank_3);
        load_usize(buff, &mut self.chr1_bank_2);
        load_usize(buff, &mut self.chr2_bank_1);
        load_usize(buff, &mut self.chr2_bank_0);
